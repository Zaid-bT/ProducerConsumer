...
static int cfg_items_limit   = 8;
static int cfg_sleep_delay   = 2;   // default 2s so wait time is visible
...
static GtkWidget* buffer_box       = NULL;
...
/* ─────────────────────────────────────────────
   CSS — bigger fonts, larger layout
   ───────────────────────────────────────────── */
static const char* APP_CSS =
    "window { background-color: #0f1117; }"
    ".root-box { background-color: #0f1117; padding: 20px; }"

    ".topbar { background-color: #1a1d27; border-radius: 12px; padding: 14px 20px; margin-bottom: 16px; }"
    ".app-title { color: #e2e8f0; font-size: 20px; font-weight: bold; font-family: 'Courier New', monospace; }"
    ".app-sub   { color: #64748b; font-size: 13px; font-family: 'Courier New', monospace; }"

    ".panel { background-color: #1a1d27; border-radius: 12px; padding: 16px; margin: 6px; }"
    ".panel-title { color: #94a3b8; font-size: 13px; font-weight: bold; font-family: 'Courier New', monospace; margin-bottom: 10px; }"

    ".slot-full  { background-color: #10b981; border-radius: 6px; min-width: 36px; min-height: 36px; }"
    ".slot-empty { background-color: #1e293b; border-radius: 6px; min-width: 36px; min-height: 36px; border: 1px solid #334155; }"

    ".thread-name  { color: #94a3b8; font-size: 14px; font-family: 'Courier New', monospace; min-width: 100px; }"
    ".thread-count { color: #64748b; font-size: 13px; font-family: 'Courier New', monospace; min-width: 50px; }"
    ".badge-run  { background-color: #064e3b; color: #10b981; border-radius: 8px; padding: 2px 10px; font-size: 12px; font-family: 'Courier New', monospace; }"
    ".badge-wait { background-color: #451a03; color: #f59e0b; border-radius: 8px; padding: 2px 10px; font-size: 12px; font-family: 'Courier New', monospace; }"
    ".badge-done { background-color: #1e293b; color: #475569; border-radius: 8px; padding: 2px 10px; font-size: 12px; font-family: 'Courier New', monospace; }"
    ".badge-idle { background-color: #1e293b; color: #475569; border-radius: 8px; padding: 2px 10px; font-size: 12px; font-family: 'Courier New', monospace; }"

    ".stat-card  { background-color: #0f172a; border-radius: 10px; padding: 12px 16px; margin: 4px; }"
    ".stat-value { color: #e2e8f0; font-size: 28px; font-weight: bold; font-family: 'Courier New', monospace; }"
    ".stat-label { color: #475569; font-size: 12px; font-family: 'Courier New', monospace; }"

    "textview { background-color: #0a0d14; }"
    "textview text { background-color: #0a0d14; color: #4ade80; font-size: 13px; font-family: 'Courier New', monospace; }"

    ".btn-start { background-color: #10b981; color: #fff; border-radius: 8px; padding: 8px 22px; font-family: 'Courier New', monospace; font-size: 14px; border: none; }"
    ".btn-start:hover { background-color: #059669; }"
    ".btn-pause { background-color: #f59e0b; color: #fff; border-radius: 8px; padding: 8px 22px; font-family: 'Courier New', monospace; font-size: 14px; border: none; }"
    ".btn-pause:hover { background-color: #d97706; }"
    ".btn-stop  { background-color: #ef4444; color: #fff; border-radius: 8px; padding: 8px 22px; font-family: 'Courier New', monospace; font-size: 14px; border: none; }"
    ".btn-stop:hover  { background-color: #dc2626; }"
    ".btn-end   { background-color: #7c3aed; color: #fff; border-radius: 8px; padding: 8px 22px; font-family: 'Courier New', monospace; font-size: 14px; border: none; }"
    ".btn-end:hover   { background-color: #6d28d9; }"

    ".speed-val { color: #e2e8f0; font-size: 14px; font-family: 'Courier New', monospace; min-width: 32px; }"
    ".speed-lbl { color: #64748b; font-size: 13px; font-family: 'Courier New', monospace; }"

    ".dialog-bg    { background-color: #0f1117; }"
    ".dialog-title { color: #e2e8f0; font-size: 22px; font-weight: bold; font-family: 'Courier New', monospace; }"
    ".dialog-sub   { color: #64748b; font-size: 13px; font-family: 'Courier New', monospace; margin-bottom: 16px; }"
    ".field-label  { color: #94a3b8; font-size: 14px; font-family: 'Courier New', monospace; min-width: 180px; }"
    ".field-hint   { color: #334155; font-size: 12px; font-family: 'Courier New', monospace; }"
    ".launch-btn   { background-color: #10b981; color: #fff; border-radius: 10px; padding: 12px 36px; font-family: 'Courier New', monospace; font-size: 14px; border: none; margin-top: 12px; }"
    ".launch-btn:hover { background-color: #059669; }"
    "spinbutton { background-color: #1a1d27; color: #e2e8f0; border-radius: 6px; border: 1px solid #334155; font-family: 'Courier New', monospace; font-size: 13px; }"
    "scale trough { background-color: #1e293b; border-radius: 4px; min-height: 6px; }"
    "scale highlight { background-color: #3b82f6; border-radius: 4px; }"
    "scale slider { background-color: #3b82f6; border-radius: 50%; min-width: 18px; min-height: 18px; border: none; }";

/* ─────────────────────────────────────────────
   GUI log: append message to the log textview
   (safe to call from any thread via g_idle_add)
   ───────────────────────────────────────────── */
typedef struct { char msg[512]; } LogMsg;

static gboolean append_log_idle(gpointer data) {
    LogMsg* lm = (LogMsg*)data;
    if (!log_textview) { free(lm); return FALSE; }
...
/* This replaces log_event for GUI output — called from producer/consumer */
void gui_log(const char* msg) {
    log_event(msg);   /* still writes to file + stdout */
...
/* ─────────────────────────────────────────────
   Refresh callback — fires every 400ms
   ───────────────────────────────────────────── */
static gboolean refresh_gui(gpointer data) {
    ...
    for (int i = 0; i < cfg_num_producers && p_stats; i++) {
        ...
        if (p_stats[i].items_produced >= cfg_items_limit) {
            ...
            badge_class = "badge-wait"; badge_text = "paused";
        } else if (stop_flag) {
...
static void on_pause_clicked(GtkButton* btn, gpointer data) {
    ...
    if (pause_flag) {
        ...
        gui_log("Simulation paused.");
    } else {
        gtk_button_set_label(GTK_BUTTON(btn_pause), "Pause");
        gui_log("Simulation resumed.");
    }
}
...
static void* join_thread_worker(void* arg) {
    ...
    g_idle_add(reenable_buttons, NULL);
    return NULL;
}
...
static void on_stop_clicked(GtkButton* btn, gpointer data) {
    ...
    pause_flag  = 0;
    sim_running = FALSE;
    gui_log("Stop signal sent — waiting for threads...");
    ...
    pthread_detach(joiner);
}

/* END button — stops threads if running, then exits the whole program */
static void on_end_clicked(GtkButton* btn, gpointer data) {
    stop_flag  = 1;
    pause_flag = 0;
    gui_log("Ending simulation and closing...");
    /* Give threads a moment to notice stop_flag, then quit */
    g_timeout_add(600, (GSourceFunc)gtk_main_quit, NULL);
}
...
static void build_main_window(void) {
    ...
    /* ── top bar ── */
    GtkWidget* topbar = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 14);
    ...
    GtkWidget* title_col = gtk_box_new(GTK_ORIENTATION_VERTICAL, 3);
    ...
    btn_end   = gtk_button_new_with_label("End");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(btn_end),   "btn-end");
    gtk_widget_set_sensitive(btn_pause, FALSE);
    gtk_widget_set_sensitive(btn_stop,  FALSE);
    ...
    g_signal_connect(btn_end,   "clicked", G_CALLBACK(on_end_clicked),   NULL);
    ...
    gtk_box_pack_start(GTK_BOX(topbar), btn_end,   FALSE, FALSE, 6);
    ...
    gtk_widget_set_size_request(speed_scale, 120, -1);
    speed_label = gtk_label_new("2s");
    ...
    gtk_box_pack_start(GTK_BOX(topbar), speed_lbl_static, FALSE, FALSE, 8);
    ...
    /* Buffer panel */
    GtkWidget* buf_panel = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
    ...
    buffer_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
    ...
    for (int i = 0; i < cfg_buffer_size; i++) {
        ...
        gtk_widget_set_size_request(slot, 36, 36);
        slot_boxes[i] = slot;
    ...
    /* Producers panel */
    GtkWidget* prod_panel = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
    ...
    for (int i = 0; i < cfg_num_producers; i++) {
        ...
        gtk_box_pack_start(GTK_BOX(row), count_lbl, TRUE,  TRUE,  0);
        gtk_box_pack_start(GTK_BOX(prod_panel), row, FALSE, FALSE, 0);
    }

    /* Consumers panel */
    GtkWidget* cons_panel = gtk_box_new(GTK_ORIENTATION_VERTICAL, 8);
    ...
    /* Log panel */
    GtkWidget* log_panel = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
    ...
    gtk_text_view_set_left_margin(GTK_TEXT_VIEW(log_textview), 8);
    gtk_text_view_set_top_margin(GTK_TEXT_VIEW(log_textview), 6);
    ...
    gtk_widget_set_size_request(right_col, 190, -1);
    gtk_box_pack_start(GTK_BOX(content), right_col, FALSE, FALSE, 0);

    struct { const char* label; GtkWidget** wptr; } cards[] = {
    ...
    refresh_timer_id = g_timeout_add(400, refresh_gui, NULL);
...
static void build_startup_dialog(void) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(startup_dialog), 480, 400);
    ...
    gtk_widget_set_margin_start(inner, 28);
    gtk_widget_set_margin_end(inner, 28);
    gtk_widget_set_margin_top(inner, 28);
    gtk_widget_set_margin_bottom(inner, 28);
    ...
    GtkWidget* sub = gtk_label_new("Producer-Consumer Problem — Restaurant Simulation");
    ...
    struct {
        ...
        { "Items per thread",  "1 – 50 orders",   1,  50,  8, &spin_items     },
    };
    for (int i = 0; i < 4; i++) {
